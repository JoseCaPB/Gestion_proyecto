<?php

namespace app\models;

use Yii;

/**
 * This is the model class for table "actividades".
 *
 * @property int $idActividad
 * @property string $nombre
 * @property int $idProyecto
 *
 * @property Proyectos $idProyecto0
 * @property ActividadesUsuarios[] $actividadesUsuarios
 * @property Incidencias[] $incidencias
 */
class Actividades extends \yii\db\ActiveRecord
{
    /**
     * {@inheritdoc}
     */
    public static function tableName()
    {
        return 'actividades';
    }

    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['nombre', 'idProyecto'], 'required'],
            [['idProyecto'], 'integer'],
            [['nombre'], 'string', 'max' => 100],
            [['idProyecto'], 'exist', 'skipOnError' => true, 'targetClass' => Proyectos::className(), 'targetAttribute' => ['idProyecto' => 'idProyecto']],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function attributeLabels()
    {
        return [
            'idActividad' => 'Id Actividad',
            'nombre' => 'Nombre',
            'idProyecto' => 'Id Proyecto',
        ];
    }

    /**
     * Gets query for [[IdProyecto0]].
     *
     * @return \yii\db\ActiveQuery
     */
    public function getIdProyecto0()
    {
        return $this->hasOne(Proyectos::className(), ['idProyecto' => 'idProyecto']);
    }

    /**
     * Gets query for [[ActividadesUsuarios]].
     *
     * @return \yii\db\ActiveQuery
     */
    public function getActividadesUsuarios()
    {
        return $this->hasMany(ActividadesUsuarios::className(), ['idActividad' => 'idActividad']);
    }

    /**
     * Gets query for [[Incidencias]].
     *
     * @return \yii\db\ActiveQuery
     */
    public function getIncidencias()
    {
        return $this->hasMany(Incidencias::className(), ['idActividad' => 'idActividad']);
    }
}
